...
<div class="card">
    <h5 class="card-header">Editer Livraison</h5>
    <div class="card-body">
      <form method="post" action="{{route('shipping.update',$shipping->id)}}">
        ...
        <input id="inputTitle" type="text" name="type" placeholder="Saisie Typa"  value="{{$shipping->type}}" class="form-control">
        @error('title')
        ...
        <input id="price" type="number" name="price" placeholder="Saisir prix"  value="{{$shipping->price}}" class="form-control">
        @error('price')
        <span class="text-danger">{{$message}}</span>
        @enderror
        </div>
